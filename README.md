...
- **⊘ Clear All** button on each date removes all sources instantly.
...
Click **+ Add Date**, pick a date from the picker, then **Add Date**. The new date appears in chronological order.
...
- Remove all sources at once with **⊘ Clear All** (undoable within 2 minutes).
...
Set **USD Rate** in the header and click **Update** (or press Enter). Only new Actual USD entries use this rate; all stored INR values remain unchanged.
...
date_records   (id, user_id → users, date, created_at)  UNIQUE (user_id, date)
